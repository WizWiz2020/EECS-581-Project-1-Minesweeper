...
GRID_WIDTH = 10
GRID_HEIGHT = 10
NUM_MINES = 3 #changed int(sys.argv[1]) to 10, no clue what sys.argv[1] is supposed to do
...
def onLeftClick(x, y): #added first_click parameter, global variable issues
    """
    ...
    """
    global first_click #added global variable to access bool
    if first_click:
        init_grid(x, y)
        first_click = False
...
def onRightClick(x, y):
    ...
    y (int): The y-coordinate of the clicked cell.
    """
    global mines_remaining #uses global variable mines remaing
    # Check if the clicked cell is within the grid bounds
    if 0 <= x < GRID_WIDTH and 0 <= y < GRID_HEIGHT:
        cell = grid[y][x]
        if not cell.is_revealed:
